...
from app.schema import DocumentInput, MessageCreate, QuickMessage, UserCreate, UserMessage
from app.services import db

from app.config import get_config
from app.factories.pg_store import AsnyPgVector
from app.factories.pgvector_store_factory import get_vector_store
from app.schema import Conversation, ConversationCreate, DocumentResponse
from app.services import crud_services as crud, langchain_service
from app.services.raptor_service import TextClusterSummarizer
from app.utils.common import load_conversation_history
...
async def save_temp_file(upload_file: UploadFile, directory: str = "/tmp",
...
async def get_documents_by_ids(ids: list[str], token: str = Depends(oauth2_scheme)):
    try:
        if isinstance(pgvector_store, AsnyPgVector):
...
@router.post("/create-conversation", response_model=Conversation)
async def create_conversation(conversation: ConversationCreate,
token: str = Depends(oauth2_scheme),
                              db_session=Depends(db.get_db)) -> Conversation:
    """ Create conversation """
...
async def quick_response(message: UserMessage, db_session=Depends(db.get_db),
token: str = Depends(oauth2_scheme)):
    Service = langchain_service.LangChainService(model_name=config.SERVICE_MODEL, template=template)
...
async def agent_response(message: UserMessage, db_session=Depends(db.get_db),
token: str = Depends(oauth2_scheme)):
    Service = langchain_service.LangChainService(model_name=config.SERVICE_MODEL, template=template)
...
@router.get("/user-conversations", response_model=List[Conversation])
async def get_user_conversations(user_sub: str, db_session=Depends(db.get_db),
token: str = Depends(oauth2_scheme)) -> List[Conversation]:
    """
...
@router.get("/get-conversation-messages", response_model=List[MessageCreate])
async def get_conversation_messages(conversation_id: str, db_session=Depends(db.get_db),
token: str = Depends(oauth2_scheme)) -> List[MessageCreate]:
    """
